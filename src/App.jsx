...
export default function App() {
  ...
  const [contributionMode, setContributionMode] = useState("pay");
  ...
  return <main className="app-shell">
    ...
    {editingContribution && <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setEditingContribution(null)}><form className="login-modal contribution-modal" onSubmit={saveContributionEdit}><button type="button" className="modal-close" onClick={() => setEditingContribution(null)}>×</button><p className="modal-kicker">Building {wing} · Flat {editingContribution.owner.flat}</p><h2>{isAdmin ? "Edit contribution" : "Give contribution"}</h2><p className="frozen-owner">{editingContribution.owner.name}</p><small>{isAdmin ? "Owner details are frozen. Only contribution information can be changed." : "Your amount is private and can only be viewed by the administrator."}</small>{!isAdmin && <div className="contribution-tabs"><button type="button" className={contributionMode === "pay" ? "active" : ""} onClick={() => { setContributionMode("pay"); setContributionEdit({ ...contributionEdit, mode: "UPI", date: today() }); }}>Scan & pay</button><button type="button" className={contributionMode === "manual" ? "active" : ""} onClick={() => setContributionMode("manual")}>Already paid</button></div>}<label>Amount<input autoFocus required type="number" min="1" inputMode="numeric" value={contributionEdit.amount} onChange={(e) => setContributionEdit({ ...contributionEdit, amount: e.target.value })} placeholder="₹ 0"/></label>{!isAdmin && contributionMode === "pay" && <div className="modal-payment"><div className="mini-qr"><QRCode value={contributionPaymentLink} size={132} type="svg" bordered={false} errorLevel="H"/></div><div><strong>Scan and pay</strong><span>{UPI_ID}</span><a className={!contributionEdit.amount ? "disabled" : ""} href={contributionEdit.amount ? contributionPaymentLink : undefined}>Pay now</a><small>Complete the UPI payment, then submit below.</small></div></div>}{(isAdmin || contributionMode === "manual") && <div className="form-row"><label>Payment mode<select value={contributionEdit.mode} onChange={(e) => setContributionEdit({ ...contributionEdit, mode: e.target.value })}><option>UPI</option><option>Cash</option><option>Bank transfer</option><option>Other</option></select></label><label>Payment date<input required type="date" value={contributionEdit.date} onChange={(e) => setContributionEdit({ ...contributionEdit, date: e.target.value })}/></label></div>}<label>Remarks<input value={contributionEdit.remarks} onChange={(e) => setContributionEdit({ ...contributionEdit, remarks: e.target.value })} placeholder={contributionMode === "manual" && !isAdmin ? "Transaction reference or note" : "Optional note"}/></label><button className="unlock-button" disabled={saving}>{saving ? "Saving…" : contributionMode === "manual" && !isAdmin ? "Submit previous payment" : "Submit privately"}</button>{isAdmin && editingContribution.existing && <button type="button" className="delete-contribution" onClick={deleteContribution} disabled={saving}>Delete incorrect entry</button>}</form></div>}
